...
func apply_time_of_day(type: String, env: WorldEnvironment, sun: DirectionalLight3D):
if type == "нощ":
...
func set_gravity(space: RID, val: float):
PhysicsServer3D.area_set_param(space, PhysicsServer3D.AREA_PARAM_GRAVITY, val)
