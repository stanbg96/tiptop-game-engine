...
func _on_confirm_and_enter():
    _save_config()
    api_page.visible = false
    _add_log("[color=#00ff88]✓ Модел:[/color] " + current_model)
...
func _on_http_response(result: int, response_code: int, headers: PackedStringArray, body: PackedByteArray):
    ...
    if is_testing_models:
        ...
        if response_code == 200:
            ...
            if json.parse(body.get_string_from_utf8()) == OK:
                var res = json.get_data()
                var ch = res.get("choices", [])
                if not ch.is_empty():
                    _interpret_and_build(ch[0].get("message", {}).get("content", ""))
                else:
...
func _render_models_list(source_list: Array, filter_query: String):
    ...
    for item in source_list:
        ...
        if m_id.is_empty() or m_id == "openrouter/free":
            continue
        if not q.is_empty() and not q in m_id.to_lower():
            continue
        if ":free" in m_id.to_lower():
            models_list_ui.add_item("🎁 [FREE] " + m_id)
            model_ids_map.append(m_id)
        else:
            models_list_ui.add_item("⭐ " + m_id)
            model_ids_map.append(m_id)
...
func _load_saved_config():
    if not FileAccess.file_exists("user://api_config.json"):
        return
    ...
    if not raw_models_cache.is_empty():
        _render_models_list(raw_models_cache, "")
...
func _on_send_chat():
    ...
    if EnvStudio.handle_command(low, env_node, sun_light, rim_light, get_world_3d().space):
        _add_log("[color=#00f2fe]Командата за света е изпълнена![/color]")
        return
...
func _get_spawn_pos() -> Vector3:
    var fwd = -camera.global_transform.basis.z
    fwd.y = 0.0
    var p = camera.global_position + fwd.normalized() * 7.5
    p.y = 0.0
    return p
...
func _spawn_archetype(arch: String, pos: Vector3):
    ...
    if arch == "piano":
        node = CAD.spawn_piano(pos, world)
    elif arch == "castle":
        node = CAD.spawn_castle(pos, world)
    elif arch == "wall":
        node = CAD.spawn_brick_wall(pos, world)
    elif arch == "car":
        node = CAD.spawn_car(pos, world, "BMW_Cabrio", "#0066ff", true)
    elif arch == "house":
        node = CAD.spawn_house(pos, world, "Cozy_House", "#e76f51")
    if node:
        _select(node)
