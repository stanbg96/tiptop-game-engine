...
func _ready():
    ...
    _add_log("[color=#00ff88]✨ TipTop Studio е онлайн и работи безупречно![/color]")
    _add_log("[color=#ffff66]💡 Опитай: 'замък', 'бемве', 'пиано', 'къща'![/color]")
    
    # Зареждаме обектите безопасно
...
func _on_send_chat():
    ...
    elif "стен" in low or "тухл" in low: _spawn_wall("Brick_Wall", pos)
    else: _spawn_house("House", "#e76f51", pos)
...
func _play_piano_chord():
    ...
    _add_log("[color=#00ff88]🎵 Пианото изсвири акорд![/color]")
...
func _spawn_car(n: String, col_hex: String, is_cab: bool, pos: Vector3):
    ...
    _col(rb, Vector3(2.4,1.3,4.6), Vector3(0,0.65,0)); world.add_child(r); _select(r); _add_log("[color=#00ff88]✓ Кола създадена![/color]")
...
func _spawn_castle(n: String, pos: Vector3):
    ...
    for tp in [Vector3(-3.8,0,-3.8), Vector3(3.8,0,-3.8), Vector3(-3.8,0,3.8), Vector3(3.8,0,3.8)]:
        ...
        var cone = CylinderMesh.new(); cone.top_radius=0.02; cone.bottom_radius=1.35; cone.height=2.2; _add_m(rb, cone, rf, tp+Vector3(0,6.6,0))
