...
static func spawn_piano(pos: Vector3, world: Node3D) -> Node3D:
    ...
    lacquer.metallic = 0.4
    lacquer.roughness = 0.06
    lacquer.clearcoat_enabled = true
    lacquer.clearcoat = 1.0

    var gold = StandardMaterial3D.new()
    gold.albedo_color = Color(0.96, 0.82, 0.28)
    gold.metallic = 0.96
    gold.roughness = 0.12

    var red_felt = _mat(Color(0.8, 0.05, 0.12))
    var wk = _mat(Color(0.96, 0.96, 0.98))
    var bk = _mat(Color(0.06, 0.06, 0.08))
...
static func spawn_castle(pos: Vector3, world: Node3D) -> Node3D:
    var root = Node3D.new()
    root.name = "Medieval_Castle"
    root.position = pos
    var rb = StaticBody3D.new()
    rb.add_to_group("prop")
    root.add_child(rb)

    var stone = _mat(Color(0.48, 0.5, 0.55), 0.95)
    var blue_roof = _mat(Color(0.12, 0.3, 0.75), 0.75)
    var wood = _mat(Color(0.28, 0.15, 0.08))
    var flag = _mat(Color(1.0, 0.15, 0.15))
    ...
    var mcone = CylinderMesh.new()
    mcone.top_radius = 0.02
    mcone.bottom_radius = 2.1
    mcone.height = 2.8
...
static func spawn_car(pos: Vector3, world: Node3D, car_name: String, color_hex: String, is_cabrio: bool) -> Node3D:
    ...
    p_mat.metallic = 0.92
    p_mat.roughness = 0.14
    p_mat.clearcoat_enabled = true
    p_mat.clearcoat = 1.0

    var blk = _mat(Color(0.1, 0.1, 0.12))
    var chrm = StandardMaterial3D.new()
    chrm.albedo_color = Color(0.9, 0.9, 0.95)
    chrm.metallic = 0.98
    chrm.roughness = 0.15
    ...
    if is_cabrio:
        ...
        gm.albedo_color = Color(0.1, 0.25, 0.4, 0.4)
        gm.transparency = BaseMaterial3D.TRANSPARENCY_ALPHA_DEPTH_PRE_PASS
        _box(rb, Vector3(1.85, 0.48, 0.06), gm, Vector3(0, 0.92, -0.45), Vector3(deg_to_rad(-24), 0, 0))

        var sm = _mat(Color(0.15, 0.15, 0.18), 0.6)
        _box(rb, Vector3(0.65, 0.5, 0.6), sm, Vector3(-0.45, 0.72, 0.2))
        _box(rb, Vector3(0.65, 0.5, 0.6), sm, Vector3(0.45, 0.72, 0.2))
    ...
    _box(rb, Vector3(2.0, 0.06, 0.35), blk, Vector3(0, 0.96, 1.85))

    var hl = StandardMaterial3D.new()
    hl.albedo_color = Color(1, 1, 1)
    hl.emission_enabled = true
    hl.emission = Color(0.85, 0.95, 1)
    hl.emission_energy_multiplier = 4.0
    _box(rb, Vector3(0.42, 0.14, 0.05), hl, Vector3(-0.7, 0.56, -2.26))
    _box(rb, Vector3(0.42, 0.14, 0.05), hl, Vector3(0.7, 0.56, -2.26))

    var tl = StandardMaterial3D.new()
    tl.albedo_color = Color(1, 0, 0)
    tl.emission_enabled = true
    tl.emission = Color(1, 0, 0)
    tl.emission_energy_multiplier = 4.0
    _box(rb, Vector3(0.55, 0.12, 0.05), tl, Vector3(-0.65, 0.6, 2.26))
    _box(rb, Vector3(0.55, 0.12, 0.05), tl, Vector3(0.65, 0.6, 2.26))
...
static func spawn_brick_wall(pos: Vector3, world: Node3D) -> Node3D:
    var root = Node3D.new()
    root.name = "Brick_Wall"
    root.position = pos
    var rb = StaticBody3D.new()
    rb.add_to_group("prop")
    root.add_child(rb)

    var br = _mat(Color(0.72, 0.25, 0.16), 0.92)
    var bd = _mat(Color(0.58, 0.18, 0.12), 0.95)
    var mt = _mat(Color(0.7, 0.7, 0.72))
    var sc = _mat(Color(0.85, 0.85, 0.88), 0.85)

    _box(rb, Vector3(6.0, 3.2, 0.55), mt, Vector3(0, 1.6, 0))
    for r in range(7):
        var yp = 0.25 + r * 0.45
        var ox = 0.3 if r % 2 == 1 else 0.0
        for c in range(-4, 5):
            var xp = c * 0.7 + ox
            if abs(xp) < 2.8:
                _box(rb, Vector3(0.62, 0.38, 0.62), br if (r + c) % 2 == 0 else bd, Vector3(xp, yp, 0))
...
static func spawn_house(pos: Vector3, world: Node3D, house_name: String, wall_col: String) -> Node3D:
    ...
    var sm = _mat(Color(0.3, 0.32, 0.36), 0.95)
    var rm = _mat(Color(0.48, 0.15, 0.12), 0.8)
    var wdm = _mat(Color(0.28, 0.16, 0.08), 0.7)
    var wfm = _mat(Color(0.95, 0.95, 0.95))
    var gm = StandardMaterial3D.new()
    gm.albedo_color = Color(0.2, 0.4, 0.6, 0.5)
    gm.transparency = BaseMaterial3D.TRANSPARENCY_ALPHA_DEPTH_PRE_PASS
    ...
    for wp in [Vector3(-1.5, 1.8, -2.24), Vector3(1.5, 1.8, -2.24), Vector3(-2.54, 1.8, 0), Vector3(2.54, 1.8, 0)]:
        ...
        _box(rb, Vector3(1.0, 1.0, 0.08), gm, wp)
...
static func compile_recipe(recipe: Dictionary, pos: Vector3, world: Node3D) -> Node3D:
    var root = Node3D.new()
    root.name = str(recipe.get("name", "Object"))
    root.position = pos
    var rb = StaticBody3D.new()
    rb.add_to_group("prop")
    root.add_child(rb)
    ...
    for p in recipe.get("parts", []):
        ...
        m.metallic = float(p.get("metallic", 0.2))
        m.roughness = float(p.get("roughness", 0.4))
        ...
        if sh == "cylinder":
            _cyl(rb, p_sz.x * 0.5, p_sz.y, m, p_pos, p_rot)
        elif sh == "sphere":
            _sph(rb, p_sz.x * 0.5, m, p_pos, p_rot)
        elif sh == "torus":
            _torus(rb, p_sz.x * 0.5, max(0.02, p_sz.x * 0.15), m, p_pos, p_rot)
        elif sh == "prism":
            _prism(rb, p_sz, m, p_pos, p_rot)
        else:
            _box(rb, p_sz, m, p_pos, p_rot)
...
static func _box(p: Node3D, sz: Vector3, mat: Material, pos: Vector3, rot: Vector3 = Vector3.ZERO):
    var m = BoxMesh.new()
    m.size = sz
    _add_mesh(p, m, mat, pos, rot)

static func _cyl(p: Node3D, r: float, h: float, mat: Material, pos: Vector3, rot: Vector3 = Vector3.ZERO):
...
static func _sph(p: Node3D, r: float, mat: Material, pos: Vector3, rot: Vector3 = Vector3.ZERO):
    var m = SphereMesh.new()
    m.radius = r
    m.height = r * 2.0
    _add_mesh(p, m, mat, pos, rot)

static func _torus(p: Node3D, orad: float, irad: float, mat: Material, pos: Vector3, rot: Vector3 = Vector3.ZERO):
    var m = TorusMesh.new()
    m.outer_radius = orad
    m.inner_radius = irad
    _add_mesh(p, m, mat, pos, rot)

static func _prism(p: Node3D, sz: Vector3, mat: Material, pos: Vector3, rot: Vector3 = Vector3.ZERO):
    var m = PrismMesh.new()
    m.size = sz
    _add_mesh(p, m, mat, pos, rot)

static func _col(p: Node3D, sz: Vector3, pos: Vector3):
...
# Дублиращ псевдоним за 100% защита от грешки при извикване
static func _add_m(p: Node3D, msh: Mesh, mat: Material, pos: Vector3, rot: Vector3 = Vector3.ZERO):
    _add_mesh(p, msh, mat, pos, rot)
